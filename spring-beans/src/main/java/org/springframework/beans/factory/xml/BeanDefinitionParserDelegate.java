...
public class BeanDefinitionParserDelegate {
	...
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele) {
		return parseBeanDefinitionElement(ele, null);// mu
	}
	 ...
	 * {@link org.springframework.beans.factory.parsing.ProblemReporter}.
	 */
	@Nullable// mu 重要 解析Bean定义元素
	public BeanDefinitionHolder parseBeanDefinitionElement(Element ele, @Nullable BeanDefinition containingBean) {
		String id = ele.getAttribute(ID_ATTRIBUTE);// mu 1、获取bean标签中，属性id和name的值
		String nameAttr = ele.getAttribute(NAME_ATTRIBUTE);

		List<String> aliases = new ArrayList<>(); // 2、将属性name的值，通过分割符","或";"进行切分，并将数据添加到aliases中
		if (StringUtils.hasLength(nameAttr)) {
			String[] nameArr = StringUtils.tokenizeToStringArray(nameAttr, MULTI_VALUE_ATTRIBUTE_DELIMITERS);
			aliases.addAll(Arrays.asList(nameArr));
		}

		String beanName = id;// mu 3、如果属性id的值为空，那就取aliases集合中的第一个value中的值，作为bean的名称
		...
		// mu 4、开始深度解析bean标签，将解析的结果封装为AbstractBeanDefinition
		...
		if (beanDefinition != null) {
			...
			String[] aliasesArray = StringUtils.toStringArray(aliases);// mu 5、根据解析道德参数，创建一个BeanDefinitionHolder
	...
	public AbstractBeanDefinition parseBeanDefinitionElement(
		...
		if (ele.hasAttribute(CLASS_ATTRIBUTE)) { // mu 1、如果bean标签存在class属性，获取class属性的值
			className = ele.getAttribute(CLASS_ATTRIBUTE).trim();
		}
		String parent = null;
		if (ele.hasAttribute(PARENT_ATTRIBUTE)) {// mu 2、如果bean标签存在parent属性，获取parent属性的值
			parent = ele.getAttribute(PARENT_ATTRIBUTE);
		}

		try {// mu 3、通过属性class和parent的值，初步创建AbstractBeanDefinition
			AbstractBeanDefinition bd = createBeanDefinition(className, parent);
			// mu 4、解析bean标签中的各种其他属性，并封装到AbstractBeanDefinition
			parseBeanDefinitionAttributes(ele, beanName, containingBean, bd);
			bd.setDescription(DomUtils.getChildElementValueByTagName(ele, DESCRIPTION_ELEMENT));
			// mu 5、解析bean标签下的各种子标签元素，将解析结果封装到AbstractBeanDefinition中
			parseMetaElements(ele, bd);// mu 解析bean的子标签:meta
			parseLookupOverrideSubElements(ele, bd.getMethodOverrides());// mu 解析bean的子标签:lookup-method
			parseReplacedMethodSubElements(ele, bd.getMethodOverrides());// mu 解析bean的子标签:replace-method

			parseConstructorArgElements(ele, bd);// mu 解析bean的子标签: constructor-arg
			parsePropertyElements(ele, bd);// mu 解析bean的子标签: property
			parseQualifierElements(ele, bd);// mu 解析bean的子标签: qualifier
	 ...
	 */ // mu 重要 解析bean标签中的各种其他属性，并封装到AbstractBeanDefinition中
	...
	/**
	 * Parse the meta elements underneath the given element, if any.解析给定元素下面的元元素(如果有的话)
	 */
	public void parseMetaElements(Element ele, BeanMetadataAttributeAccessor attributeAccessor) {
		NodeList nl = ele.getChildNodes();// mu 获取bean标签下的所有子节点
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			if (isCandidateElement(node) && nodeNameEquals(node, META_ELEMENT)) {// mu 遍历找到meta标签
	...
	public void parseConstructorArgElements(Element beanEle, BeanDefinition bd) {
		...
		for (int i = 0; i < nl.getLength(); i++) {
			Node node = nl.item(i);
			if (isCandidateElement(node) && nodeNameEquals(node, CONSTRUCTOR_ARG_ELEMENT)) {// mu 找到名称为constructor-arg的标签
