...
public class XmlBeanDefinitionReader extends AbstractBeanDefinitionReader {
	 ...
	 * if none specified. // mu 实体解析器
	 */
	protected EntityResolver getEntityResolver() {
		if (this.entityResolver == null) { //entityResolver果然是在getEntityResolver方法中初始化的
			// Determine default EntityResolver to use.
			ResourceLoader resourceLoader = getResourceLoader();
			if (resourceLoader != null) {
				this.entityResolver = new ResourceEntityResolver(resourceLoader); // mu ResourceEntityResolver
			}
			else {
				this.entityResolver = new DelegatingEntityResolver(getBeanClassLoader()); // mu DelegatingEntityResolver
			}
		}
	...
	public int loadBeanDefinitions(Resource resource) throws BeanDefinitionStoreException {
		// mu 将resource 封装成EncodedResource
	...
	protected int doLoadBeanDefinitions(InputSource inputSource, Resource resource)
		...
		try {
			// mu 根据输入流和Resource，创建xml文件对应的Document
			Document doc = doLoadDocument(inputSource, resource);
			// mu 解析Document 并注册bean到spring容器中
